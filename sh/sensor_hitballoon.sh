...
sleep 3s
gnome-terminal --tab  -e 'bash -c "source $HOME/catkin_ws/devel/setup.bash; python3 $HOME/catkin_ws/src/object_det/scripts/det.py; exec bash"'
sleep 5s
gnome-terminal --tab  -e 'bash -c "source $HOME/catkin_ws/devel/setup.bash; roslaunch faster_lio rflysim_sim.launch rviz:=false; exec bash;"'
sleep 3s
gnome-terminal --tab  -e 'bash -c "source $HOME/catkin_ws/devel/setup.bash; roslaunch ego_planner RflySimDepth.launch; exec bash"'
...
gnome-terminal --window  -e 'bash -c "rostopic echo /mavros/state; exec bash;"'
