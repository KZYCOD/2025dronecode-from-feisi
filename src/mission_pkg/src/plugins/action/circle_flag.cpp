...
PortsList CircleFlag::providedPorts()
{
  return {
    ...
    InputPort<Position3D>("search_area", Position3D(5, 5, 2), "搜索区域中心"),
    InputPort<double>("circle_radius", 2.0, "环绕半径"),
    InputPort<double>("circle_speed", 1.5, "环绕速度"),
    InputPort<double>("circle_height", 2.0, "环绕高度")
  };
}
