...
class Hit : public StatefulActionNode
{
  enum Method
  {
...
public:
  ...
  Hit(const std::string &name, const NodeConfig &config);
  ...
  bool getVel(double &vx, double &vy, double &vz,double &yaw_rate);
...
private:
  ...
  bool is_vis_servo_end; // 视觉伺服刺的末端以及看不见目标，只能直接去刺破
};
